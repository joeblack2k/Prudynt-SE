...
std::vector<ConfigItem<const char *>> CFG::getCharItems()
{
    return {
        ...
        {"stream1.mode", stream1.mode, "CBR", [](const char *v) {
            std::set<std::string> a = {"CBR", "VBR", "SMART", "FIXQP", "CAPPED_VBR", "CAPPED_QUALITY"};
            return a.count(std::string(v)) == 1;
        }},
...
std::vector<ConfigItem<int>> CFG::getIntItems()
{
    return {
        ...
        {"stream1.fps", stream1.fps, 30, validateInt120},
        {"stream1.gop", stream1.gop, 30, validateIntGe0},
        {"stream1.height", stream1.height, 720, validateIntGe0},
        ...
        {"stream1.width", stream1.width, 1280, validateIntGe0},
...
bool CFG::serializeConfig(std::string &out)
{
    config_loaded = readConfig();
