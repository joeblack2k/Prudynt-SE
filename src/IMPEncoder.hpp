...
public:
    ...
    IMPEncoder(_stream *stream, int encChn, int encGrp, int fsChn, const char *name) : stream(stream), encChn(encChn), encGrp(encGrp), fsChn(fsChn), name(name)
    {
        init();
    }
...
private:
    ...
    void initProfile();
    bool ownsGroupResources() const { return encGrp == encChn; }
