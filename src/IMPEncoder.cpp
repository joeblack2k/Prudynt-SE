...
IMPEncoder *IMPEncoder::createNew(
    ...
    const char *name)
{
    return new IMPEncoder(stream, encChn, encGrp, fsChn, name);
}
...
int IMPEncoder::init()
{
    LOG_DEBUG("IMPEncoder::init(encChn=" << encChn << ", encGrp=" << encGrp
              << ", fsChn=" << fsChn << ")");
...
#if defined(PLATFORM_T31) || defined(PLATFORM_C100) || defined(PLATFORM_T40) || defined(PLATFORM_T41)
    if (stream->allow_shared)
    {
        int share_from = -1;
        if (cfg->stream2.enabled && cfg->stream2.jpeg_channel == encChn)
            share_from = 2;
        else if (cfg->stream3.enabled && cfg->stream3.jpeg_channel == encChn)
            share_from = 3;

        if (share_from >= 0)
        {
            ret = IMP_Encoder_SetbufshareChn(share_from, encChn);
            LOG_DEBUG_OR_ERROR_AND_EXIT(ret, "IMP_Encoder_SetbufshareChn(" << share_from << ", " << encChn << ")");
        }
    }
#endif
    ...
    if (strcmp(stream->format, "JPEG") != 0 && ownsGroupResources())
    {
    ...
    LOG_INFO("IMPEncoder ready: encChn=" << encChn
             << " encGrp=" << encGrp
             << " fsChn=" << fsChn
             << " format=" << stream->format
             << " size=" << stream->width << "x" << stream->height
             << " fps=" << static_cast<int>(stream->fps)
             << " gop=" << stream->gop);
