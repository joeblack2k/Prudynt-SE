...
{
    int r = 0;         // current requests
    int rps = 0;       // requests per second
...
namespace
{
int clamp_jpeg_channel(int channel)
{
    if (channel < 0 || channel >= NUM_JPEG_CHANNELS)
        return 0;
    return channel;
}

bool jpeg_channel_enabled(int channel)
{
    channel = clamp_jpeg_channel(channel);
    return channel == 0 ? cfg->stream2.enabled : cfg->stream3.enabled;
}
}

bool get_snapshot(std::vector<unsigned char> &image, int channel = 0)
{
    channel = clamp_jpeg_channel(channel);

    if (!jpeg_channel_enabled(channel))
    {
        LOG_ERROR("Preview requested but JPEG capture is disabled for channel " << channel);
        return false;
    }

    {
        std::unique_lock<std::mutex> lock_stream{mutex_main};
        if (!global_jpeg[channel]->snapshot_buf.empty())
        {
            image.resize(LWS_PRE + global_jpeg[channel]->snapshot_buf.size());
            std::memcpy(image.data() + LWS_PRE,
                        global_jpeg[channel]->snapshot_buf.data(),
                        global_jpeg[channel]->snapshot_buf.size());
            return true;
        }
    }

    std::ifstream file(global_jpeg[channel]->stream->jpeg_path, std::ios::binary);
    if (!file.is_open())
    {
        LOG_ERROR("Failed to open snapshot file " << global_jpeg[channel]->stream->jpeg_path << ": "
                                                 << strerror(errno));
        return false;
    }
    ...
    LOG_ERROR("Snapshot file is empty: " << global_jpeg[channel]->stream->jpeg_path);

    return false;
}

bool WS::get_snapshot_bytes(std::vector<unsigned char> &image, int channel)
{
    image.clear();
    ...
    if (!jpeg_channel_enabled(channel))
    {
        LOG_ERROR("Snapshot requested but JPEG capture is disabled for channel " << channel);
        return false;
    }

    global_jpeg[channel]->request();
    if (!global_jpeg[channel]->active)
    {
        global_jpeg[channel]->should_grab_frames.notify_all();
        const auto activation_timeout = milliseconds(cfg->general.imp_polling_timeout
                                                     + cfg->websocket.first_image_delay + 250);
        if (!global_jpeg[channel]->is_activated.try_acquire_for(activation_timeout))
        {
            LOG_ERROR("Timed out waiting for JPEG worker activation");
            return false;
        }
    }

    const auto deadline = steady_clock::now()
                          + milliseconds(cfg->general.imp_polling_timeout
                                         + cfg->websocket.first_image_delay + 250);
    while (steady_clock::now() < deadline)
    {
        if (get_snapshot(image, channel))
            return true;
...
signed char WS::rtsp_callback(struct lejp_ctx *ctx, char reason)
{
    ...
    if (reason & LEJP_FLAG_CB_IS_VALUE && ctx->path_match)
    {
        ...
        if (restart_flag && restart_threads_by_signal(restart_flag) < 0)
        {
            LOG_ERROR("Dropping RTSP restart request because another restart is already in progress");
        }
...
signed char WS::action_callback(struct lejp_ctx *ctx, char reason)
{
    ...
    if (reason & LEJP_FLAG_CB_IS_VALUE && ctx->path_match)
    {
        ...
        case PNT_SAVE_CONFIG:
            cfg->updateConfig();
            add_json_str(u_ctx->message, pnt_ws_msg[PNT_WS_MSG_OK]);
            break;
        case PNT_DUMP_CONFIG:
            add_json_str(u_ctx->message, pnt_ws_msg[PNT_WS_MSG_OK]);
            break;
        case PNT_CAPTURE:
            ...
            {
                LOG_ERROR("Rejecting websocket preview request because stream2 JPEG capture is disabled");
                add_json_str(u_ctx->message, pnt_ws_msg[PNT_WS_MSG_ERROR]);
            }
            break;
        default:
...
bool WS::process_json_message(const std::string &request, std::string &response)
{
    response.clear();
    ...
    if (request.find("\"dump_config\"") != std::string::npos)
    {
        std::ifstream config_stream("/etc/prudynt.json");
        if (!config_stream.is_open())
        {
            response = "{\"error\":\"config_open_failed\"}";
            return false;
        }
    ...
    user_ctx u_ctx(generateSessionID(), nullptr);
    lejp_ctx ctx;

    u_ctx.message = "{";
    lejp_construct(&ctx, root_callback, &u_ctx, root_keys, LWS_ARRAY_SIZE(root_keys));
    const int parse_ret = lejp_parse(&ctx,
...
static void
send_ws_snapshot(lws_sorted_usec_list_t *sul)
{
...
static void
send_http_snapshot(lws_sorted_usec_list_t *sul)
{
    struct user_ctx *u_ctx = lws_container_of(sul, struct user_ctx, sul);
    lws_callback_on_writable(u_ctx->wsi);
}

static bool ensure_preview_capture_ready(int &wake_delay_us)
{
    wake_delay_us = 0;

    if (!cfg->stream2.enabled)
    {
        LOG_ERROR("Preview requested but stream2 JPEG capture is disabled");
        return false;
    }
    ...
    {
        wake_delay_us = cfg->websocket.first_image_delay * 1000;
        global_jpeg[0]->should_grab_frames.notify_all();

        const auto activation_timeout = milliseconds(cfg->general.imp_polling_timeout
                                                     + cfg->websocket.first_image_delay + 250);
        if (!global_jpeg[0]->is_activated.try_acquire_for(activation_timeout))
        {
            LOG_ERROR("Timed out waiting for JPEG worker activation");
            return false;
        }
...
static int compute_ws_preview_delay_us(user_ctx *u_ctx, int wake_delay_us)
{
    u_ctx->snapshot.r++;
    ...
    int source_fps = global_jpeg[0]->stream->stats.fps;
    if (source_fps < 1)
        source_fps = 1;

    int effective_fps = source_fps + u_ctx->snapshot.throttle;
    if (effective_fps < 1)
        effective_fps = 1;

    return (LWS_USEC_PER_SEC / effective_fps) + wake_delay_us;
}
...
int WS::ws_callback(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len)
{
    ...
    case LWS_CALLBACK_RECEIVE:
        ...
        if (u_ctx->flag & PNT_FLAG_WS_REQUEST_PREVIEW)
        {
            ...
            int wake_delay_us = 0;
            if (!ensure_preview_capture_ready(wake_delay_us))
            {
                u_ctx->flag &= ~PNT_FLAG_WS_PREVIEW_PENDING;
            ...
            int delay = compute_ws_preview_delay_us(u_ctx, wake_delay_us);
            LOG_DDEBUGWS("shedule preview image. id:" << u_ctx->id << " delay:" << delay);
            lws_sul_schedule(lws_get_context(wsi), 0, &u_ctx->sul, send_ws_snapshot, delay);
    ...
    case LWS_CALLBACK_HTTP:
    {
        ...
        if (strcmp(token, url_token) == 0 || (strcmp(cfg->websocket.token, "auto") != 0 && strcmp(cfg->websocket.token, "") != 0 && strcmp(cfg->websocket.token, url_token) == 0))
        {
            LOG_DEBUG("HTTP request authenticated and user context initialized");
        }
        else
        {
            ...
            {
                LOG_DEBUG("Allowing unauthenticated connection (http_secured=false)");
            }
        }
        ...
        if (request_method == 0)
        {
            ...
            if (strcmp(url_ptr, "/preview.jpg") == 0)
            {
                ...
                if (wake_delay_us > 0)
                {
                    lws_sul_schedule(lws_get_context(wsi), 0, &u_ctx->sul, send_http_snapshot,
                                     wake_delay_us);
                    return 0;
                }
