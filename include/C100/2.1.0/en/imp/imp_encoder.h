 ...
 * @{
 */
#define	IMP_NR_MAX_ENC_CHN_IN_GROUP 3  /**<the max channel amount in group of encoder*/
...
/**
* Define the reference type of coded frame
*/
typedef enum {
  IMP_ENC_SHORT_TERM_REF = 0,				/**< current frame is short term ref frame*/
  IMP_ENC_IS_LONG_TERM_REF = 0x02,			/**< current frame is long term ref frame*/
  IMP_ENC_USE_LONG_TERM_REF = 0x04,			/**< current frame is frame that use long term ref frame*/
  IMP_ENC_MARKING_REF_MAX_ENUM = 0xff,		/**< max index, no practical meaning*/
}IMPEncoderRefType;

typedef struct {
	/**<channel register state in encoder group. 1 to already registered, 0 to not register.
		eg. achnregistedstat[0] == 1, indicate that channel_0 has been registered in group.*/
	int achnregistedstat[IMP_NR_MAX_ENC_CHN_IN_GROUP];
}IMPEncoderChnRegStatusInGrp;
...
typedef struct {
	...
	IMPEncoderRefType erefType;         /**< referenct type of coded frame */
} IMPEncoderStream;
...
/**
 * @brief IMP_Encoder_GroupWhichChnRegisted(const int encGroup,IMPEncoderChnRegStatusInGrp *pChnRegStat);
 *
 * get registered channel state 
 *
 * @param[in] encGroup       index of encode  group
 * @param[out] pChnRegStat   register state of channel in the group
 *
 * @retval   0   success
 * @retval  <0   failed
 *
 ...
 * @attention none.
 */
int IMP_Encoder_GroupWhichChnRegisted(const int encGroup,IMPEncoderChnRegStatusInGrp *pChnRegStat);
